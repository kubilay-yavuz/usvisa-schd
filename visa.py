...
GMAIL_API_KEY = config['NOTIFICATION']['GMAIL_API_KEY']
...
def send_notification(title, msg):
    ...
    if GMAIL_API_KEY:
        ...
        subject = title
        body = msg
        sender = "USVISA@gmail.com"
        recipients = ["GMAIL_API_KEY@gmail.com", "USVISA@gmail.com"]
        password = "123123123"

        def _send_email(subject, body, sender, recipients, password):
            _msg = MIMEText(body)
            _msg['Subject'] = subject
            _msg['From'] = sender
            _msg['To'] = ', '.join(recipients)
            with smtplib.SMTP_SSL('smtp.gmail.com', 465) as smtp_server:
               smtp_server.login(sender, password)
               smtp_server.sendmail(sender, recipients, _msg.as_string())
            print("Message sent!")


        _send_email(subject, body, sender, recipients, password)
    if PUSHOVER_TOKEN:
...
def reschedule(date):
    ...
    time.sleep(STEP_TIME)
    send_notification('Time found', 'book quick!')
    ...
    if(r.text.find('Successfully Scheduled') != -1):
        title = "SUCCESS"
        msg = f"Rescheduled Successfully! {date} {_time}"
    else:
        title = "FAIL"
        msg = f"Reschedule Failed!!! {date} {_time}"
    return [title, msg]
...
if LOCAL_USE:
    ...
    # driver = webdriver.Chrome(executable_path=r"C:\Users\kubilay.yavuz\Desktop\us_visa_scheduler-main\chrome-win32\chrome.exe")
    from selenium import webdriver
    from selenium.webdriver.chrome.service import Service
    from webdriver_manager.chrome import ChromeDriverManager
    ...
    while 1:
        ...
        try:
            ...
            try:
                ...
                info_logger(LOG_FILE_NAME, 'time out')
                time.sleep(60)
